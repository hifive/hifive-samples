	...
	h5.u.obj.expose('sample.util', {
		...
		ajax: function(url, data) {
			...
			case sample.consts.url.SEARCH_WORKFLOW:
				...
				break;
			case sample.consts.url.WORKFLOW:
				if (!purchaseDatas) {
					createPurchaseDatas(NUM_OF_WORKFLOW_DATAS);
				}
				// 配列で返す
				var ids = $.isArray(data.id) ? data.id : [data.id];
				ret = [];
				for (var i = 0, l = ids.length; i < l; i++) {
					var id = ids[i];
					var datas = null;
					if (id.indexOf('_') === 0) {
						// ダミーの大量データID('_'始まり)の場合
						datas = dummyMassDatas;
					} else {
						datas = purchaseDatas;
					}
				...
				break;
			case sample.consts.url.CONFIRM:
				// 配列で返す
				var ids = $.isArray(data.ids) ? data.ids : [data.ids];
				ret = [];
				for (var i = 0, l = ids.length; i < l; i++) {
					var id = ids[i];
					var datas = null;
